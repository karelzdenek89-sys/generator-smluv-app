...
export default function PartnerNextSteps({ offers, context, sourcePage, attributionId }: PartnerNextStepsProps) {
  ...
  useEffect(() => {
    if (eligibleSent.current || offers.length === 0) return;
    eligibleSent.current = true;
    for (const offer of offers) {
      trackEvent('partner_offer_eligible', analyticsParams(offer, context, sourcePage, attributionId));
    }
  }, [offers, context, sourcePage, attributionId]);
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver((entries) => {
      for (const entry of entries) {
        ...
        trackEvent('partner_offer_viewed', analyticsParams(offer, context, sourcePage, attributionId));
        ...
        >
          ...
          <a
            ...
            onClick={() => trackEvent('partner_offer_clicked', {
              ...analyticsParams(offer, context, sourcePage, attributionId),
