...
import {
  LANDING_COMPARISON,
  WHY_NOT_GENERIC_BULLETS,
} from '@/lib/marketing/differentiation';

type WhyNotGenericBlockProps = {
  className?: string;
  /** Např. „u nájemní smlouvy“ — doplní se do úvodní věty */
  documentHint?: string;
  compact?: boolean;
  /** Kompaktní tabulka vzor vs. SmlouvaHned */
...
export default function WhyNotGenericBlock({
  ...
  const intro = documentHint
    ? `Hledáte vzor ${documentHint}? Stažený soubor nezná vaše strany, částky ani termíny. Tady postupujete jinak.`
    : 'Hledáte vzor z internetu? Stažený soubor nezná vaše strany, částky ani termíny. Tady postupujete jinak.';

  const { generic, ours } = LANDING_COMPARISON;
  ...
  return (
    ...
    >
      ...
      <ul className={`mt-5 space-y-3 ${compact ? 'text-sm' : 'text-base'}`}>
        {WHY_NOT_GENERIC_BULLETS.map((line) => (
      ...
      <p className="mt-5 text-sm leading-relaxed text-[#8f8472]">
        SmlouvaHned je softwarový generátor, ne advokátní kancelář.{' '}
