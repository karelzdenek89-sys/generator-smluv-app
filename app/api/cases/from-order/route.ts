...
/**
 * „Pokračovat jako zakázka“ ze success stránky.
 *
 * Vstup: Stripe session_id + download token (kapabilita kupujícího). Server
 * ověří zaplacení i token a založí případ na doručovací e-mail objednávky.
 * Odpověď nese token pouze pro okamžité otevření; návratový odkaz jde e-mailem.
 */
export async function POST(req: Request) {
  ...
  if (!/^cs_[A-Za-z0-9_]{10,200}$/.test(sessionId) || !token || token.length > 200) {
  ...
  try {
    ...
    if (!result.ok) {
      const status =
        ...
        : result.reason === 'not_paid' ? 402
        : result.reason === 'unsupported' ? 422
        : 404;
      return NextResponse.json({ error: 'Zakázku z této objednávky nelze založit.' }, { status });
    }
    if (result.created) {
      await recordAnalyticsEvent('case_started', {
        ...
        case_origin: 'success_page',
        contract_type: 'work_contract',
        tier: result.record.origin.tier,
        package_key: result.record.origin.packageKey ?? undefined,
      });
    }
    return NextResponse.json({
      caseId: result.record.id,
      token: result.token,
      path: `${CASE_PAGE_PATH}?id=${encodeURIComponent(result.record.id)}`,
    ...
    return NextResponse.json({ error: 'Zakázku se nepodařilo založit. Zkuste to prosím znovu.' }, { status: 500 });
  }
}
