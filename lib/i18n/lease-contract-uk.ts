  ...
  return [
    ...
    },
    {
      ...
      body: [
        ...
        isFixedTerm
          ? 'Після закінчення строку договір припиняється, якщо сторони письмово не домовляться інакше. Дострокове розірвання допускається лише на підставах закону або цього договору. Якщо орендар користується квартирою щонайменше три місяці після закінчення оренди, а орендодавець у цей строк не вимагає звільнити квартиру, оренда поновлюється на той самий строк, але не більше ніж на два роки (§ 2285 ЦК).'
    ...
    },
    {
      ...
      body: hasDeposit
        ? [
            ...
            'Після закінчення оренди орендодавець повертає заставу або її невикористаний залишок із законними відсотками, за вирахуванням доведених і деталізованих сум, та без зайвої затримки надає розрахунок після звільнення квартири й встановлення її стану.',
