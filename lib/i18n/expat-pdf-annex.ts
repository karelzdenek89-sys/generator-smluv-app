...
const CONTRACT_LABELS: Record<ExpatContractType, { en: string; ua: string }> = {
  ...
  car_sale: { en: 'vehicle purchase agreement', ua: 'договір купівлі-продажу авто' },
};
  ...
  if (locale === 'ua' && translationAnnex) {
    if (isUaSummaryAnnex(data.contractType)) {
      return [
        ...
        'Нижче — чеський текст договору. Далі в PDF — стислий огляд основних умов (не повний переклад). У разі розбіжностей перевагу має чеське формулювання.',
      ];
    }
    return [
      'ЧЕСЬКИЙ ДОГОВІР З ПОЯСНЮВАЛЬНИМ УКРАЇНСЬКИМ ДОДАТКОМ',
      'Нижче — чеський текст договору. Далі — пояснювальний український переклад; він не є засвідченим чи офіційним. У разі розбіжностей перевагу має чеське формулювання.',
    ];
  }
...
function laborAnnexIntroUa(label: string): string {
  return [
    ...
    'У разі розбіжностей перевагу має чеське формулювання.',
    EMPLOYMENT_WORK_ELIGIBILITY_NOTICE_CS,
    EMPLOYMENT_WORK_ELIGIBILITY_NOTICE_UK,
